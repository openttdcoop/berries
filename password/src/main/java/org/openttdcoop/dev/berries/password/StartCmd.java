 ...
 */
@Rename("start")
public class StartCmd implements GrapeCommand
{
    private PasswordPlugin password;
    
    public StartCmd (PasswordPlugin password)
    {
    ...
    {
        if (this.password.task.isCancelled()) {
            this.password.startTask();
            cc.reply("Password rotation task started.");
        } else {
            cc.reply("Password rotation task already running.");
        }
    }
